...
class TCL(nn.Module):
    ...
    def __init__(self, classes, classifier_dims, lambda_epochs=1):
        """
        :param classes: Number of classification categories
        :param views: Number of views
        :param classifier_dims: Dimension of the classifier
        ...
        self.lambda_epochs = lambda_epochs
        # self.Classifiers = nn.ModuleList([Classifier(classifier_dims[i], self.classes) for i in range(self.views)])
        self.Classifiers = Classifier_v2(classifier_dims, self.classes)
    def DS_Combin(self, alpha):
        """
        :param alpha: All Dirichlet distribution parameters.
        :return: Combined Dirichlet distribution parameters.
    ...
    def forward(self, X, y, global_step):
        ...
        # alpha = dict()
        alpha = evidence + 1
        loss += ce_loss(y, alpha, self.classes, global_step, self.lambda_epochs)
        # alpha_a = self.DS_Combin(alpha)
        # evidence_a = alpha - 1
        # loss += ce_loss(y, alpha_a, self.classes, global_step, self.lambda_epochs)
        loss = torch.mean(loss)
    ...
    def infer(self, input):
        ...
        :return: evidence of every view
        """
        evidence = self.Classifiers(input)
        return evidence


class TMC_Mix(nn.Module):
    ...
    def forward(self, X, y, global_step, args):
        evidence = self.infer(X)
        loss = 0
        alpha = dict()
        for v_num in range(len(X)):
            alpha[v_num] = evidence[v_num] + 1
            if v_num == 0:
                loss += args.r_lambda*ce_loss(y, alpha[v_num], self.classes, global_step, self.lambda_epochs)
            else:
                loss += ce_loss(y, alpha[v_num], self.classes, global_step, self.lambda_epochs)
...
def defense_ens(ensemble_models, robust_model,X,y,epoch,args):
    logits= torch.zeros_like(y)
    for i, surrogate_model in enumerate(ensemble_models):
        logits[i] = surrogate_model(X)
    evidences, evidence_a, loss = robust_model(logits, y, epoch)
    return loss
