...
class ResNet_v1(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, tau=0.1, image_size=(32, 32), args=None):
        ...
        self.tmc = TMC(num_classes ,self.views ,self.dim ,args.epoch)
    ...
    def forward(self, x, labels, epoch,is_eval=False):
        ...
        evidences, evidence_b, tmc_loss = self.tmc(tmc_f,labels,epoch,is_eval)
        self.evidences = evidences
        alpha, b, S, E, u = dict(), dict(), dict(), dict(), dict()
        E=evidences
        for v in range(len(E)):
            ...
            u[v] = out.size(1)/S[v]
        self.u_sum=self.tmc.u_sum
        self.u_individual=u
        self.belief_individual = b
        self.b_sum=self.tmc.b_sum
        return out, r_outputs, nr_outputs, rec_outputs,evidence_b, tmc_loss


def ResNet18_trobust_mot(num_classes=10, tau=0.1, image_size=(32, 32),args=None):
