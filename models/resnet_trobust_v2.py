...
class ResNet_v1(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, tau=0.1, image_size=(32, 32), args=None):
        ...
        self.dim = args.dims
        self.tmc = TCL(num_classes,self.dim ,args.epoch)
    ...
    def forward(self, x,labels,epoch,is_eval=False):
        ...
        out = r_feat + rec_feat
        evidences, tmc_loss = self.tmc(torch.nn.AdaptiveAvgPool2d(1)(rec_feat).reshape(rec_feat.shape[0], -1),labels,epoch)

        out = nn.AdaptiveAvgPool2d(1)(out)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        # tmc_f.append(out)
        return out, r_outputs, nr_outputs, rec_outputs, evidences,tmc_loss
