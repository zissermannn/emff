...
from defense import TMC,TCL,TMC_Mix
...
class ResNet_v1(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, tau=0.1, image_size=(32, 32), args=None):
        ...
        self.tmc = TMC_Mix(num_classes ,self.views ,self.dim ,args.epoch)
        # self.aux2 = nn.Sequential(nn.Linear(128, num_classes))
        # self.aux3 = nn.Sequential(nn.Linear(256, num_classes))
        # self.aux4 = nn.Sequential(nn.Linear(512, num_classes))
    ...
    def forward(self, x,labels,epoch,is_eval=False):
        ...
        out = r_feat + rec_feat
        tmc_f.append(torch.nn.AdaptiveAvgPool2d(1)(r_feat).reshape(r_feat.shape[0], -1))
        for _ in range(1,self.views):
            tmc_f.append(torch.nn.AdaptiveAvgPool2d(1)(rec_feat).reshape(rec_feat.shape[0], -1))
        evidences, evidence_b, tmc_loss = self.tmc(tmc_f,labels,epoch,self.args)
...
def ResNet18_trobust(num_classes=10, tau=0.1, image_size=(32, 32),args=None):
    return ResNet_v1(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, tau=tau, image_size=image_size,args=args)
