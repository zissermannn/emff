...
class VGG(nn.Module):
    def __init__(self, features, num_classes=10, init_weights=True, tau=0.1, image_size=(32, 32), args=None):
    ...
    def forward(self, x,labels,epoch, is_eval=False):
        ...
        tmc_f = []
        x = self.block0(x)
        tmc_f.append(torch.nn.AdaptiveAvgPool2d(1)(x).reshape(x.shape[0], -1))
        x = self.pool0(x)
        x = self.block1(x)
        tmc_f.append(torch.nn.AdaptiveAvgPool2d(1)(x).reshape(x.shape[0], -1))
        x = self.pool1(x)
        ...
        return x, r_outputs, nr_outputs, rec_outputs, evidence_b,tmc_loss
...
def vgg16_trobust(pretrained=False, progress=True, tau=0.1, num_classes=10, image_size=(32, 32), **kwargs):
