...
from trades import trades_loss_ours
...
parser.add_argument('--save_name', type=str, default='robust_0.25',help='specify checkpoint save name')
...
parser.add_argument('--dataset', type=str, default='imagenette', help='target dataset')
...
def train(epoch):
    ...
    with tqdm(total=(len(trainset) - len(trainset) % args.bs)) as _tqdm:
        _tqdm.set_description('TRADES {} {} {} (Train) Epoch: {}/{}'.format(args.dataset,args.model, args.save_name, epoch, args.epoch))
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            inputs, targets = inputs.to(device), targets.to(device)
            loss,loss_robust,logits_adv = trades_loss_ours(net,inputs,targets,optimizer,epoch,step_size=args.alpha * (args.eps/255.0),epsilon=args.eps/255.0,perturb_steps=10,beta=args.beta)
            # net.eval()
            ...
            # loss = adv_cls_loss + tmc_loss
            # optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            adv_cls_losses += loss_robust.item()
    ...
    torch.save(net.state_dict(), './weights/{}/{}/TRADES_{}.pth'.format(args.dataset, args.model, args.save_name+'_'+args.optim+'_'+str(args.use_lr_schedule)+'_'+str(args.epoch)))
